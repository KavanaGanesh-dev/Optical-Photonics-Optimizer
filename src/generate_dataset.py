...
def generate_dataset(n_samples=5000, path= "/Users/kavanakiran/Documents/AI_Projects/optical-photonics-optimizer/data/optical_channel_dataset.csv"):
    ...
    for i in tqdm(range(n_samples), desc='simulating data for optical channel'):
        wavelength = np.random.uniform(1530, 1570)  #wavelngth in nm - narrow more realistic
        launch_power = np.random.uniform(-15,5)    #power in dBm - more negative - means harder
        temperature = np.random.uniform(15,70)      #temp in celsius - wide temp range
